...
const proposalResolver = require("../resolvers/proposalResolver");
const responseStatus = require("../../config/responseStatuses");

router.get("/", async (req, res, next) => {
  try {
    const proposals = await proposalResolver.getAllProposals();
    res.status(responseStatus.success).json({ proposals });
  } catch (err) {
    next(err);
  }
});
...
router.get("/:id", async (req, res, next) => {
  ...
  try {
    ...
    res.status(responseStatus.success).json({ proposal });
  } catch (err) {
    if (TypeError) {
      console.log(err);
